...
- **SC-005**: 폴러 사이클 1회 실행 시, 조회된 PENDING 배치가 모두 그 사이클 내에 발행 시도됨을 통합 테스트로 검증한다. (운영 환경의 실제 평균 발행 지연은 폴링 주기에 의해 결정되며 — 기본 5초 주기 → 평균 ≈ 2.5초, 최악 ≈ 5초 + 발행 시간 — `test` 프로필은 폴러 자동 실행이 비활성이라 주기 기반 지연을 직접 측정하지 않고 per-cycle 완전성으로 갈음한다. analyze U1 반영.)
