...
### R4. 버전 해석: 매 요청 DB 파생 (메모이즈 안 함)

- **Decision**: `getCurrentVersion()` 은 **매 요청 DB 에서 직접** 파생한다(별도 버전 캐시 없음). `job_log(job_type, end_time)` 복합 인덱스를 추가해 `MAX(endTime)` 을 인덱스 끝값 1건 읽기로 처리.
- **Rationale**:
  - 막아주는 비용이 인덱스된 `MAX` 스칼라 1건(≈0.1ms)에 불과 — 마이페이지 트래픽 수준에서 캐싱으로 아낄 가치가 낮다(조기 최적화 회피).
  - 메모 제거 시 **stale 창이 0 으로 수렴**(배치 완료 즉시 버전 반영), 동시 조회 레이스 경계도 "메모 만료 instant"(≤TTL) → "DB 커밋 instant"(마이크로초)로 축소.
  - 코드 단순화(캐시 1개 제거), 정확성↑.
- **Alternatives**:
  - 짧은 TTL(5~60s) 메모 — 트래픽 폭주 시 DB 부하 절감되나 stale 창·레이스 창을 그만큼 도입. **실측 부하가 확인되면 그때 재도입**(후속 최적화로 보류).
  - 영구/긴 캐시 — 버전 전환 미감지. 기각.
- **인덱스 근거**: 단조 증가하는 `end_time` 의 MAX 를 인덱스 역방향 스캔 1건으로 → 풀스캔 회피.
